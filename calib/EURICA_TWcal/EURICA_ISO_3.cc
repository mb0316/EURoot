void EURICA_ISO_3()
{
	...
	Int_t temp_ch;
	Double_t gcT_iso_TW[84][3];
	ifstream calib;
	calib.open ("gcT_iso_TW.dat");
	
	for (Int_t i = 0; i < 84; i++)	calib >> temp_ch >> gcT_iso_TW[i][0] >> gcT_iso_TW[i][1] >> gcT_iso_TW[i][2];
	...
	for (Long64_t ient = 0; ient < nEnt; ient++)
	{
		...
		{
			if (gc_T[ihit] > -500E3 && gc_T[ihit] < 500E3)    gc_T[ihit] = -(gc_T[ihit] - gcT_iso_TW[gc_ch[ihit]][0] - gcT_iso_TW[gc_ch[ihit]][1]*TMath::Power(gc_E[ihit], -gcT_iso_TW[gc_ch[ihit]][2]));
	...
	ofstream gtc;
	gtc.open("iso_gamma_time_cut.dat");
	for (Int_t i = 0; i < 50; i++)
	{
