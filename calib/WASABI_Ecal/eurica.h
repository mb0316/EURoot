...
// Wed Sep 28 12:40:19 2016 by ROOT version 5.34/21
// from TTree tree/go4 tree
// found on file: SiCalib_0012.root
//////////////////////////////////////////////////////////
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("SiCalib_0012.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("SiCalib_0012.root");
      }
...
eurica::eurica(char* filename, TTree* tree)
{
   if (tree == 0) {
...
{
	std::cout << "start finding EURICA Ts." << std::endl;
	Long64_t nentries = fChain->GetEntriesFast();
	Long64_t nbytes = 0, nb = 0;
	Long64_t ts;
	cout << nentries << " emtries." << endl;
	for (Long64_t jentry=0; jentry<nentries;jentry++)
	{
		...
		if (jentry%100000 == 0)
		{
			cout << jentry << " done." << endl;
			cout << ts << " " << jentry << endl;
			cout << "size of map : " << mts.size() << endl;
		}
	}
}
