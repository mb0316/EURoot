...
//	tree->SetBranchAddress("add_T", add_T);
	...
	for (Int_t i = 0; i < 5; i++)
	{
		for (Int_t j = 0; j < 60; j++)	hist_chX[i][j] = new TH1D(Form("dssd_x%d_ch%d",i+1,j+1), "", 2000, 0, 4000);
		for (Int_t j = 0; j < 40; j++)	hist_chY[i][j] = new TH1D(Form("dssd_y%d_ch%d",i+1,j+1), "", 2000, 0, 4000);
	}
			...
			for (Int_t isi = 0; isi < 5; isi++)
			{
				...
				if (dssdhitY == 1)
				{
					for (Int_t ich = 0; ich < 40; ich++)
					{
						sitime = beta_T_Y[isi][ich];
						//if (getime > 21000 && getime < 22500 && sitime > -50000 && beta_E_Y[isi][ich] >= 200 && add_E[i] >= 400 && add_E[i] <= 1000)
						if (getime > 21558.5 && getime < 21796.1 && sitime < dssd_Y_Tcut_H[isi] && sitime > dssd_Y_Tcut_L[isi] && beta_E_Y[isi][ich] >= 200 && ge_E[i] > 400 && ge_E[i] < 1100)
						{
	...
	TFile* out = new TFile("SiEcal_Esum.root", "recreate");
	out -> cd();
	for (Int_t i = 0; i < 5; i++)
	{
