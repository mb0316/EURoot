...
EUTreeDecay::EUTreeDecay(const char* filename, TTree *tree)
{
    if (tree == 0)
    {
...
EUTreeDecay::EUTreeDecay(TTree* tree)
{
    ...
    tree -> Branch("beta_E", &beta_E, "beta_E/I");
...
void EUTreeDecay::Init(TTree *tree)
{
    if (!tree) return;
    fData = tree;
    fCurrent = -1;
